...
### mtk64el になりました
   ...
   ロープロファイルキーはGateron Low Profile 2.0 (KS-33) を使用します。（Kailh chock v1は使用できません）
...
[MBK Choc Low-Profile Keycaps](https://shop.yushakobo.jp/products/mbk-choc-low-profile-keycaps)のような、スフィリカル（お椀状に凹んでいる形状）のキーでは手前側の角が親指に当たって痛くなってしまう方におすすめです。（mtk64elでは使用できません）
</details>

## キーボードの接続

まず、動作確認のために右左の各キーボードを単体でPCに接続し、キー入力が可能であること、OLEDにキーボードの情報が表示されることを確認してください。（OLEDは一定時間操作しないと消灯します）

USB Type-c ケーブルは充電器や携帯電話などの製品に付属のケーブルは使用できません。USB2.0/USB3.0の表記のあるデータ通信用のケーブルを使用してください。
PCからキーボードとして認識されない場合、USBケーブルがデータ通信用ではない、USBケーブルがケースと干渉してプロマイクロにしっかり刺さっていないなどの可能性があります。

根元の形状が太いUSBケーブルや特殊な形状のUSBケーブルは、ケースと干渉する場合があります。
作例は[こちら](https://amzn.asia/d/gQuaIi1)のUSBケーブルを使用しています。

USB2.0のケーブルを使用してType-c端子にケーブルをきちんと差し込んだたうえで、接続時にキー入力できない場合やOLEDに何も表示されない場合、ロゴが表示されていてキー情報（数値やコード）が表示されない場合は、OLEDの斜め下にあるリセットボタン（ボルトの頭）をプッシュするか、キーボードを接続し直してください。

USBケーブルに問題がなく、動作しない場合は、プロマイクロのコンスルーが抜けかけている、回路がショートしていて起動できない（LEDの取り付け方向間違いでこの状態になることが多いです）などのハードウェアの問題か、ファームウェアが消えているなどのソフトウェアの問題が考えられます。
...
ジャックの付け根が太いケーブルやL字のケーブルはケースに干渉するため、根元の細いケーブルを使用するか、ケース側を加工する必要があります。
作例は[こちら](https://shop.dailycraft.jp/products/trrs_cable_50_gloss)のTRRSケーブルを使用しています。

スリープからの復帰の際にキーボードが動作しなくなる場合、PC側のUSB省電力設定やUSBセレクティブサスペンドをOFFにするか、OLED斜め下のリセットボタン押下、USBケーブルの再接続で復帰してください。
